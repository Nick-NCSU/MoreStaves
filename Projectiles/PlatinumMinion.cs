...
{
	// Adds the Platinum Minion as a projectile.
	public class PlatinumMinion : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Platinum");
		...
		public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<Buffs.PlatinumBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<Buffs.PlatinumBuff>()))
			{
				...
				if (foundTarget)
				{
					...
					for (int x = -10; x <= 10; x += 5)
                    {
						for(int y = -10; y <= 10; y += 10)
                        {
							...
							Projectile.NewProjectile(position, velocity, ModContent.ProjectileType<PlatinumProjectile>(), 8, 2, projectile.owner);
						}
                    }
				}
			}
