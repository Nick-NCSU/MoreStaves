...
{
	// Adds the wood staff to the game
	public class WoodStaff : ModItem
	{
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			Projectile.NewProjectile(position, Vector2.Zero, ModContent.ProjectileType<Projectiles.WoodStaffProjectile>(), 0, 0, item.owner);
			// Adds the buff
		...
		public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float scale, int whoAmI)
		{
			...
			Texture2D glow = mod.GetTexture("Items/WoodGlow");
