...
{
	// Adds the tungsten staff to the game
	class TungstenStaff : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Tungsten Staff");
			Tooltip.SetDefault("Summons a tungsten minion to fight for you.");
		...
		public override void SetDefaults()
		{
			...
			item.summon = true;
			item.knockBack = 3f;
			// Spawns the Tungsten Buff
			item.buffType = ModContent.BuffType<Buffs.TungstenBuff>();
			// Shoots a Tungsten Minion
			item.shoot = ModContent.ProjectileType<Projectiles.TungstenMinion>();
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			Projectile.NewProjectile(position, Vector2.Zero, ModContent.ProjectileType<Projectiles.TungstenStaffProjectile>(), 0, 0, item.owner);
			// Adds the buff
		...
		public override void PostDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, float rotation, float scale, int whoAmI)
		{
			...
			Texture2D glow = mod.GetTexture("Items/TungstenGlow");
